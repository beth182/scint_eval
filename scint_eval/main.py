...
def main(obs_site, DOYstart, DOYstop, variable, savepath, saveyn, run, instrument, sample,
    ...
    # sort observations
    # observations.py
    obs = observations_new_scint.sort_obs_new_scint(variable, files_obs, DOYstart, DOYstop, obs_site, z0zdlist, saveyn, savepath, sample,
                                instrument)
    ...
    # finding observation files
    # file_read.py
    files_obs_kdown = file_read.finding_files(model_format, 'obs', DOYstart, DOYstop, 'KSSW', run, 'CNR4', '15min',
                                              'kdown', obs_level,
    ...
    # sort observations
    # observations.py
    obs_kdown = observations.sort_obs('kdown', files_obs_kdown, DOYstart, DOYstop, 'KSSW', z0zdlist, saveyn, savepath,
                                      '15min', 'CNR4')
    ...
    # time series plot
    # times_series.time_series_plot('kdown', saveyn, model_site_kdown, DOYstart, DOYstop, savepath + 'all_', run,
    #                               included_grids_kdown, group_obs_kdown)
    ...
    for grid_choice in included_grids_kdown.keys():
        mod_time, mod_vals = array_retrieval.retrive_array_model(included_grids_kdown, grid_choice)

        model_grid_vals_kdown[grid_choice] = mod_vals
        model_grid_time_kdown[grid_choice] = mod_time

    time_eval_kdown, obs_vals_eval_kdown, mod_vals_eval_kdown = array_retrieval.take_common_times(obs_time_kdown, obs_vals_kdown,
                                                                                model_grid_time_kdown['WAverage'],
                                                                                model_grid_vals_kdown['WAverage'])

    zeff = look_up.scint_zeff[scint_path][0]

    plotting_funs.detailed_time_series(obs_time_kdown, obs_vals_kdown,
                                       time_eval_kdown, obs_vals_eval_kdown,
                                       model_grid_time_kdown, model_grid_vals_kdown,
                                       'kdown', zeff, savepath, DOYstart, DOYstop,
                                       model_site_dict,
